...
class Zftp(ftplib.FTP):
    ...
    def list_jes_spool(self, jobmask='', owner='', status='ALL'):
        ...
        except (ZftpError, ftplib.all_errors) as msg:
    ...
    def submit_wait_job(self, jcl,jfile, jobname='', cntlfile='', spoolback=None,
                        purge = False, timeout=None):
        """
        ...
        try:
            ...
            upfile = open(jfile, "rb")
            #print ("TYPE")
            #print (type(upfile))
            resp = self.storlines('STOR ' + cntlfile,upfile)  
